...
main (int argc, char *argv[])
{
  ...
  DsdvHelper dsdv;
  dsdv.Set ("PeriodicUpdateInterval", TimeValue (Seconds (16)));
  ...
  Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> (("dsdv2.routes"), std::ios::out);
  dsdv.PrintRoutingTableAllAt (Seconds (16), routingStream);
  // We create the channels first without any IP addressing information
 ...
 // Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
  ...
  ApplicationContainer serverApps = echoServer.Install (c.Get (6));
  serverApps.Start (Seconds (17.0));
  serverApps.Stop (Seconds (20.0));
  
  UdpEchoClientHelper echoClient (i5i6.GetAddress (1), 9);
  echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
  echoClient.SetAttribute ("PacketSize", UintegerValue (1024));

  ApplicationContainer clientApps = echoClient.Install (c.Get (1));
  clientApps.Start (Seconds (18.0));
