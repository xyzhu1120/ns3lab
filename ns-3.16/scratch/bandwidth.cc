...
int main (int argc, char *argv[])
{
  ...
  std::string input ("topology/fattree.dat");

  int trans[1000];
  std::ifstream transfile;
  transfile.open("topology/fattree-trans");
  int a,b;
  while(transfile >> a >> b){ 
	trans[a] = b;
  }
  ...
  for(int i = 0; i < 27; ++i){
  Ptr<Node> randomServerNode = nodes.Get (trans[i]);
  ...
  ApplicationContainer apps = server.Install (nodes.Get (trans[i]));
  apps.Start (Seconds (1.0));
  apps.Stop (Seconds (900.0));
...
//
  ...
  apps = client.Install (nodes.Get (trans[53-i]));
