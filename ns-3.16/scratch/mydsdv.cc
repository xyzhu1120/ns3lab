...
DsdvManetExample::CreateDevices (std::string tr_name)
{
  ...
  for(unsigned int i = 0; i < m_nWifis; i++) { 
    for(unsigned int j = 0; j < m_nWifis; j++) { 
		if(i>=j)
			continue;
		if((i==0) && (j!=1)) {
			continue;
		}
		if((i==m_nWifis-1)&&(j!=m_nWifis-2))
			continue;
		if( j - i !=1 )
			continue;
		std::cout<<"link between "<<i<<" "<<j<<"\n";
...
DsdvManetExample::InstallApplications ()
{
  ...
  for (uint32_t clientNode = m_nWifis-1; clientNode <= m_nWifis - 1; clientNode++ )
  //uint32_t clientNode=m_nSinks-1;
    {
      uint32_t j = 0;
